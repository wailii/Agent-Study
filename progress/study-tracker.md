...
Supervisor 在具体框架（如 LangGraph）中的节点/边实现方式 | 中 | 未解决 | 目前只在概念和规则层面清晰，还没有在具体框架里实现过 Supervisor 节点和路由逻辑
Supervisor 与现有 RAG 系统的意图分流（检索/生成/改写等）如何融合 | 中 | 未解决 | 需要把今天的路由心智模型嫁接到已有的 RAG 意图识别与分流策略上
